...
import { BASE_SLOTS, coveredSlots, isOpen } from "./schedule";
import {
  SEED_CONTENT,
  SEED_FAQS,
  SEED_PHOTOS,
...
// Every slot id a booking occupies. Older bookings predate `slotIds`, so
// fall back to recomputing the coverage from time + hours.
function bookingSlotIds(booking) {
  if (booking.slotIds?.length) return booking.slotIds;
  return coveredSlots(booking.time, booking.hours).map((t) => slotId(booking.date, t));
}
...
export async function blockDateRange(fromIso, toIso) {
  const dates = [];
  const d = new Date(fromIso + "T00:00:00");
  const end = new Date(toIso + "T00:00:00");
  while (d <= end) {
    const iso = d.toISOString().slice(0, 10);
    if (isOpen(iso)) dates.push(iso);
  ...
  const pairs = dates.flatMap((date) => BASE_SLOTS.map((time) => ({ date, time })));
  let created = 0;
...
export async function rescheduleBooking(booking, date, time) {
  ...
  const cover = coveredSlots(time, booking.hours);
